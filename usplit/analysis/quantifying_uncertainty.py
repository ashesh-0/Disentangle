...
def _compute_metrics(data1, data2, metric_types: List[str]):
    ...
    for metric_type in metric_types:
        ...
        if metric_type == 'RMSE':
            ...
            metric = np.array([RangeInvariantPsnr(data1[0], data2[0]), RangeInvariantPsnr(data1[1], data2[1])])
        output[metric_type] = metric
    return output


def compute_regionwise_metric_pairwise(model,
                                       dset,
                                       idx_list: List[int],
                                       metric_types,
                                       regionsize: int = 64,
        ...
        for idx1 in range(sample_count):
            ...
            for idx2 in range(sample_count):
                ...
                output[img_idx]['pairwise_metric'][idx1][idx2] = compute_regionwise_metric_pairwise_one_pair(
                    rec_list[idx1], rec_list[idx2], metric_types, regionsize)
    return output
...
def normalize_metric_single_target(metric_dict: Dict[str, dict], normalize_type: str,
...
def normalize_metric(metric_dict: Dict[int, dict], normalize_type: str,
                     target_dict: Dict[int, np.ndarray]) -> Dict[int, dict]:
    """
    Args:
...
def get_regionwise_metric(model,
    ...
    if normalize_type is not None:
        upscale_metric = normalize_metric(upscale_metric, normalize_type, target_dict)
