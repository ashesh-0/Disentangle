...
def get_config():
    ...
    model = config.model
    model.model_type = ModelType.LadderVaeWithTransformer
    model.z_dims = [128, 128, 128, 128, 128]

    # will be ignored if transformer encoder is used, howver we still need to set it.
    ...
    model.transformer_encoder_params = ml_collections.ConfigDict()
    model.transformer_encoder_params.num_blocks = [2, 2, 3, 5, 2]            # L
    model.transformer_encoder_params.channels = [64, 96, 192, 384, 768]  
    model.transformer_encoder_params.block_types=['C', 'C', 'T', 'T']
