...
class LadderVAETexDiscrim(LadderVAE):
    ...
    def training_step(self, batch: tuple, batch_idx: int, log_fn=None):
        if log_fn is None:
        ...
        log_fn('reconstruction_loss', recons_loss, on_epoch=True)
        # log_fn('kl_loss', kl_loss, on_epoch=True)
        log_fn('training_loss', net_loss, on_epoch=True)
        log_fn('D_loss', D_loss, on_epoch=True)
        log_fn('L1_generated_probab', critic_dict['avg_Label1']['generated'], on_epoch=True)
        log_fn('L1_actual_probab', critic_dict['avg_Label1']['actual'], on_epoch=True)
        log_fn('L2_generated_probab', critic_dict['avg_Label2']['generated'], on_epoch=True)
        log_fn('L2_actual_probab', critic_dict['avg_Label2']['actual'], on_epoch=True)
        if mask.sum() > 0:
