...
class LadderVaeMultiDatasetMultiBranch(LadderVaeMultiDataset):
    ...
    def training_step(self, batch, batch_idx, enable_logging=True):
        ...
        out = torch.concatenate([out_mixrecons, out_2ch], dim=0)
        target_normalized = torch.concatenate([target_normalized[mask_mixrecons], target_normalized[mask_2ch]], dim=0)
        dset_idx = torch.concatenate(dset_idx[mask_mixrecons], dset_idx[mask_2ch], dim=0)
        loss_idx = torch.concatenate(loss_idx[mask_mixrecons], loss_idx[mask_2ch], dim=0)
...
if __name__ == '__main__':
    ...
    dset_idx = torch.Tensor([0, 0, 1, 0])
    loss_idx = torch.Tensor([LossType.Elbo, LossType.Elbo, LossType.ElboMixedReconstruction, LossType.Elbo])
