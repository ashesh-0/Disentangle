...
def compute_for_one(patch_predictions, gt, percentile_bins = 100, calib_stats_dict=None):
    output = []
...
def get_calibration_coverage_data(model, dset, percentile_bins = 100, num_workers=4, batch_size = 32, mmse_count = 10, calib_stats_dict=None):
    ...
    with torch.no_grad():
        for batch in tqdm(dloader):
            ...
            true_error_quantiles = compute_for_one(samples, tar_normalized.cpu().numpy(), percentile_bins=percentile_bins, calib_stats_dict=calib_stats_dict)
