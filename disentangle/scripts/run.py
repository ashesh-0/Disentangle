...
flags.DEFINE_string("datadir",
                    '/tmp2/ashesh/ashesh/VAE_based/data/MNIST/noisy/',
                    "Data directory.")
flags.DEFINE_boolean("use_max_version", False,
                     "Overwrite the max version of the model")
flags.DEFINE_string(
    "load_ckptfpath", '',
...
def main(argv):
    ...
    cur_workdir, relative_path = get_workdir(config, FLAGS.workdir,
                                             FLAGS.use_max_version)
    ...
    if FLAGS.mode == "train":
        ...
        train_data, val_data = create_dataset(config,
                                              FLAGS.datadir,
        ...
        if config.data.sampler_type == SamplerType.DefaultSampler:
            ...
            if config.data.sampler_type == SamplerType.RandomSampler:
                ...
                train_sampler = SingleImgSampler(train_data,
                                                 config.training.batch_size)
                val_sampler = SingleImgSampler(val_data,
                                               config.training.batch_size)
                ...
                val_sampler = NeighborSampler(val_data,
                                              config.training.batch_size,
                                              nbr_set_count=0)
        ...
        train_network(train_dloader, val_dloader, data_mean, data_std, config,
                      'BaselineVAECL', FLAGS.logdir)
