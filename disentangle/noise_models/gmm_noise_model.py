"""
Adapted from
https://github.com/juglab/HDN/blob/1561e882849c85a1ee67995f0c3a0315e1bf0670/lib/gaussianMixtureNoiseModel.py
"""
import torch
...
class GaussianMixtureNoiseModel:
    """The GaussianMixtureNoiseModel class describes a noise model which is parameterized as a mixture of gaussians.
       If you would like to initialize a new object from scratch, then set `params`= None and specify the other
       parameters as keyword arguments. If you are instead loading a model, use only `params`.
            ...
            **kwargs: keyworded, variable-length argument dictionary.
            Arguments include:
                ...
                weight : array
                    A [3*n_gaussian, n_coeff] sized array containing the values of the weights describing the noise
                    model. Each gaussian contributes three parameters (mean, standard deviation and weight), hence the
                    number of rows in `weight` are 3*n_gaussian.
                    If `weight=None`, the weight array is initialized using the `min_signal` and `max_signal`
                    parameters.
                n_gaussian: int
                    Number of gaussians.
                n_coeff: int
                    Number of coefficients to describe the functional relationship between gaussian parameters and the
                    signal.
                ...
                params: dictionary
                    ...
                    While initializing a new object of the class `GaussianMixtureNoiseModel` from scratch, set this to
                    `None`.

            Example
            -------
            >>> model = GaussianMixtureNoiseModel(min_signal = 484.85, max_signal = 3235.01, path='../../models/',\
            weight = None, n_gaussian = 3, n_coeff = 2, min_sigma = 50, device = torch.device("cuda:0"))
    """
    ...
    def likelihood(self, observations, signals):
        ...
        for gaussian in range(self.n_gaussian):
            p += self.normalDens(observations, gaussianParameters[gaussian],
                                 gaussianParameters[self.n_gaussian + gaussian]) * gaussianParameters[
                     2 * self.n_gaussian + gaussian]
        return p + self.tol
    ...
    def train(self, signal, observation, learning_rate=1e-1, batchSize=250000, n_epochs=2000, name='GMMNoiseModel.npz',
              lowerClip=0, upperClip=100):
